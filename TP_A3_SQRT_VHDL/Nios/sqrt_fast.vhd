library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

-- number to test : 0, 1, 512, 1194877489, 4294967295

entity sqrt_fast is
    generic(
        n : integer :=32 -- nombre de bits des entrées de sorties
    );
    port(
        CLK : in STD_LOGIC; -- clock
		CLK_EN : in STD_LOGIC; --clock enable
        RESET : in STD_LOGIC; -- Demarre le calcul sur front descendant de RESET
        start : in STD_LOGIC; --bit de start
        DATAA : in STD_LOGIC_VECTOR(2*n - 1 downto 0); -- nombre entier non signé dont on veut calculer la racine carrée
        RESULT : out STD_LOGIC_VECTOR(n - 1 downto 0); -- racine carrée de DATAA
        DONE : out STD_LOGIC -- = 1 lorsque le calcul est fini
    );
end entity sqrt_fast;


architecture arch of sqrt_fast is
    signal D : unsigned(2*n - 1 downto 0);   
    signal Z : unsigned(n - 1 downto 0);
    signal CNT : unsigned(n - 1 downto 0);
    
    type state is (wait_state, exec_state, end_state);
    signal current_state, next_state : state;
    
begin

CSM: process(clk, RESET) -- current state memorization
  begin
  if (RESET = '1') then
    current_state <= wait_state;
  elsif (clk'event and (clk = '1') and (CLK_EN = '1') then
    current_state <= next_state;
  end if;
end process CSM;

NSC: process(current_state,start) -- next state computation
  begin
  case current_state is
    when wait_state => if start = '1' then
                          next_state <= exec_state;
                       else
                          next_state <= wait_state;
                       end if;
    when exec_state => if CNT = N then
                          next_state <= end_state;
                      else
                          next_state <= exec_state;
                    end if; 
  when end_state => if start = '0' then
                        next_state <= wait_state;
                    else 
                        next_state <= end_state;  
                    end if;
end case;          
end process NSC;

sqrt_sequential: process(current_state)
	variable R : signed(2*n - 1 downto 0);
    begin
      case current_state is
        when wait_state =>
            D <= unsigned(A);
            R := to_signed(0,2*n);
            Z <= to_unsigned(0,n);
            CNT <=  to_unsigned(0,n);
            DONE <= '0';
        when exec_state =>
            if R >= to_signed(0,2*n) then
              R := (R sll 2) + signed(shift_right(D, 2*n - 2)) - shift_left(signed(resize(Z, 2*n)), 2) - to_signed(1, 2*n);
            else
              R := (R sll 2) + signed(shift_right(D, 2*n - 2)) + shift_left(signed(resize(Z, 2*n)), 2) + to_signed(3, 2*n);
            end if;
  		
            if R >= to_signed(0,2*n) then
              Z <= (Z sll 1) + 1;
            else 
              Z <= (Z sll 1);
            end if;
            D <= D Sll 2;
              
         when end_state =>     
              CNT <= CNT + 1;
            RESULT <= std_logic_vector(Z);
            DONE <='1';
    end case;
    end process sqrt_sequential;
    
    
end architecture arch;

